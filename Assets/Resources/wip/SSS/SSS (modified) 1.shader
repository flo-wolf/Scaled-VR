Shader "Custom/SSS (modified)2" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_MainTex ("Albedo (RGB)", 2D) = "white" {}
		_SmoothnessTex ("Albedo (RGB)", 2D) = "white" {}
	...
	SubShader {
		...
		sampler2D _MainTex, _BumpMap, _Ramp, _SmoothnessTex;
		...
		inline fixed4 LightingSSSTranslucent(SurfaceOutputStandard s, float3 viewDir, UnityGI gi)
		{
			...
			half litSide = smoothstep(0.2,0.2, NdotL);
			half d = dot (normal, lightDir) * 0.5 + 0.5;

			half3 ramp = tex2D(_Ramp, float2(d,d)).rgb;

			// Rim
			float rim = (1 - dot(viewDir, normal)) * 1 - litSide;

			fixed4 c = LightingStandard(s, viewDir, gi); // original color 
			c.rgb = s.Albedo * litSide * gi.light.color + s.Albedo * gi.indirect.diffuse * (1 - smoothstep(0.35,0.35, NdotL));
			//c.rgb += smoothstep(0.8, 0.8,rim) * 1;
