...
public sealed class Mathfx
{
    ...
    //Boing
    public static float BerpExtreme(float start, float end, float value)
    {
        value = Mathf.Clamp01(value);
        value = (Mathf.Sin(value * Mathf.PI * (0.5f + 2.5f * value * value * value)) * Mathf.Pow(1f - value, 2.2f) + value) * (1f + (1.5f * (1f - value)));
        return start + (end - start) * value;
    }
