Shader "Change/Triplanar" {
    Properties {
        _MainTex ("Albedo (RGB)", 2D) = "white" {}
		[NoScaleOffset] _BumpMap("Normal Map", 2D) = "bump" {}
		[NoScaleOffset] _RoughnessMap("Roughness", 2D) = "black" {}
		_Color ("Color", Color) = (1,1,1,1)
		_GlossinessTex("Smoothness (Texture)", Range(0, 1)) = 0
		...
		_EmissionStrength("Strength", Range(0.0, 1.0)) = 0
    }
    SubShader {
        ...
        sampler2D _BumpMap;
        sampler2D _OcclusionMap, _EmissionMap, _RoughnessMap;

        half _Glossiness;
        half _Metallic;
        
        half _OcclusionStrength, _EmissionStrength, _GlossinessTex;
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			// roughness textures
            half roughX = tex2D(_RoughnessMap, uvX).g;
            half roughY = tex2D(_RoughnessMap, uvY).g;
            half roughZ = tex2D(_RoughnessMap, uvZ).g;
            half rough = LerpOneTo(roughX * triblend.x + roughY * triblend.y + roughZ * triblend.z, _GlossinessTex);

			// emission textures
            half emitX = tex2D(_EmissionMap, uvX).g;
            half emitY = tex2D(_EmissionMap, uvY).g;
            half emitZ = tex2D(_EmissionMap, uvZ).g;
            half emit = LerpOneTo(emitX * triblend.x + emitY * triblend.y + emitZ * triblend.z, 1 - _EmissionStrength);
            ...
            o.Smoothness = rough * _Glossiness;
            o.Occlusion = occ;
			o.Emission = emit;
